...
本コースではPythonで作成したマルチエージェントシミュレーションを行い，結果をArcGIS Proで図化する事が目的です．
マルチエージェントシミュレーションは個々のエージェントがそれぞれのルールによって行動し，体外に影響し合って全体の振る舞いをシミュレートする方法です．
...
ワークショップ終了後はプロジェクトフォルダごとUSBメモリなどにコピーすればお持ち帰り頂くことができます．
...
ファイルエクスプローラーでプロジェクタフォルダが開きますので，ダウンロードした4個のファイル（`aoi.geojson`，`blda.geojson`，`road_polygons.geojson`，`wa.geojson`）をコピーします．
...
GeoJSONをファイルジオデータベースにピーするには，ジオプロセッシングウインドウから「JSON→フィーチャ」を選び，入力JSONにプロジェクトフォルダーにコピーしたサンプルデータを指定します．出力フィーチャクラスにGISDAY2023.gdbを指定し，サンプルデータのファイル名を入力します．ジオメトリタイプはポリゴンのままにします．他の3個のサンプルデータも同様にファイルジオデータベースファイルにコピーします．
...
1. 下図のように道路ラスタとコスト距離ラスタがあります．
2. 道路ラスタのうち値が0のメッシュはエージェントが入れないメッシュ，1のメッシュはエージェントが入れるメッシュです．
...
8. これを繰り返すことで，注目している避難者エージェントは青矢印のルートを移動します．
9. 移動先の候補にゴール地点のメッシュがあった場合にゴールしたと判定します．
...
道路ラスタとコスト距離ラスタの二つはaoiポリゴンと道路ポリゴンに必要な属性を追加し，ラスタ化してします．
...
出力場所をGISDAY2023.gdb，出力ポイントフィーチャクラスを`start_points500`，制限フィーチャクラスを`road_polygons`，ポイント数を500，最小距離を1メートルにします．実行すると選択された道路ポリゴン内にランダムなポイントが500個作成されます．
このポイントをスタート地点にします．
...
スタート地点のデータを読み込んでいる部分に`.sample(200)`を加えます．
...
ただし，道路データとコスト距離ラスタを連動して作成しないと，コス路距離ラスタに従って移動しようとした先が道路以外になっていたときなどに避難者エージェントが動けなくなってしまうことがあります．
